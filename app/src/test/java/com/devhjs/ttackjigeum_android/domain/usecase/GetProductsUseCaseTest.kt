...
class GetProductsUseCaseTest {
    ...
    @Test
    fun `invoke success returns matched products together with target price from config`() = runTest {
        val productId = 1L
        ...
        assertEquals(configTargetPrice, products[0].targetPrice) // Verify targetPrice is updated
    }

    @Test
    fun `invoke returns error when exception occurs`() = runTest {
        // Given
        coEvery { userConfigRepository.getAllUserConfigs() } throws RuntimeException("DB Error")
